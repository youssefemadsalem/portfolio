      ...
      if (rect.top <= window.innerHeight && rect.bottom >= 0) {
        ...
        glowRefs.current.forEach((node, index) => {
          if (node) {
            ...
            const yOffset = scrollDistance * speed;
  ...
  return (
    ...
    >
      {activeGradients.map((gradient, index) => (
        <div key={index} className={`absolute ${gradient.position}`}>
          <div
            ref={(el) => (glowRefs.current[index] = el)}
            // Removed rounded-full so 'ellipse' gradient shape works properly on wide divs
